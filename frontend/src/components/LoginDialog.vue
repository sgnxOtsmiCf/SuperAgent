    ...
    <div class="login-dialog">
      ...
      <el-tabs v-model="activeTab" class="login-tabs">
        ...
        <el-tab-pane label="账号密码" name="password">
          ...
          <el-form v-else ref="registerFormRef" :model="registerFormData" :rules="registerRules" @submit.prevent>
            ...
            <el-form-item prop="captchaCode">
              <div class="captcha-input-wrapper">
                <el-input
                  v-model="registerFormData.captchaCode"
                  placeholder="请输入验证码"
                  prefix-icon="Key"
                  size="large"
        ...
        <el-tab-pane label="手机号" name="phone">
          <!-- 手机号登录表单 -->
          <el-form v-if="isLogin" ref="phoneFormRef" :model="phoneFormData" :rules="phoneRules">
            ...
            <el-form-item prop="captchaCode">
              <div class="captcha-input-wrapper">
                <el-input
                  v-model="phoneFormData.captchaCode"
          ...
          <!-- 手机号注册表单 -->
          <el-form v-else ref="phoneRegisterFormRef" :model="phoneRegisterFormData" :rules="phoneRegisterRules">
            <el-form-item prop="phone">
              <el-input
            ...
            <el-form-item prop="captchaCode">
              <div class="captcha-input-wrapper">
                <el-input
                  v-model="phoneRegisterFormData.captchaCode"
                  placeholder="请输入图片验证码"
                  prefix-icon="Key"
                  size="large"
                />
            ...
            <el-form-item prop="verifyCode">
              <div class="code-input-wrapper">
                <el-input
                  v-model="phoneRegisterFormData.verifyCode"
                  placeholder="请输入短信验证码"
                  prefix-icon="Message"
                  size="large"
                />
...
// ========== 手机号注册表单数据 ==========
const phoneRegisterFormData = reactive({
  phone: '',
  verifyCode: '',
  captchaCode: ''
})
...
function switchToRegister() {
 ...
 // 清除验证提示
  ...
  if (phoneRegisterFormRef.value) {
    phoneRegisterFormRef.value.clearValidate()
  }
...
async function handleRegister() {
  ...
  try {
    const res = await userApi.register({
      ...
      captchaId: captchaId.value,
      captchaCode: registerFormData.captchaCode
    })
...
async function sendPhoneRegisterCode() {
  ...
  try {
    await phoneRegisterFormRef.value.validateField('phone')
  } catch (error) {
    return
  }

  if (!phoneRegisterFormData.captchaCode) {
    ElMessage.warning('请先输入图片验证码')
    return
  }

  try {
    await userApi.loginWithPhoneCodePre(phoneRegisterFormData.phone)
    ElMessage.success('验证码已发送！')
...
async function handlePhoneLogin() {
  ...
  try {
    const res = await userApi.login({
      ...
      captchaId: captchaId.value,
      captchaCode: phoneFormData.captchaCode
    })
...
async function handlePhoneRegister() {
  ...
  try {
    await phoneRegisterFormRef.value.validate()
  } catch (error) {
    return
  }
  ...
  try {
    const res = await userApi.register({
      registerType: 'phone',
      phone: phoneRegisterFormData.phone,
      verifyCode: phoneRegisterFormData.verifyCode,
      captchaId: captchaId.value,
      captchaCode: phoneRegisterFormData.captchaCode
    })
