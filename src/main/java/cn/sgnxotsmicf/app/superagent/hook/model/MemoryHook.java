 ...
 * @Version: 4.0
 * @Description: 企业级动态用户画像记忆Hook，基于LLM进行增量事实提取与合并，支持时间衰减与容量淘汰
 */
@Component
@HookPositions(value = {HookPosition.BEFORE_MODEL, HookPosition.AFTER_MODEL})
public class MemoryHook extends ModelHook {
    private static final Logger logger = LoggerFactory.getLogger(MemoryHook.class);
    ...
    public CompletableFuture<Map<String, Object>> afterModel(OverAllState state, RunnableConfig config) {
        ...
        try {
            ...
            if (currentUserInput == null || currentUserInput.trim().length() < MIN_INPUT_LENGTH) {
                logger.debug("用户输入过短或为空，跳过画像提取, userId: {}", userId);
                return CompletableFuture.completedFuture(Map.of());
            }
            Store store = config.store();
            Optional<StoreItem> itemOpt = store.getItem(List.of(USER_PROFILE_NAMESPACE), userIdStr);
            ...
            String userContent = String.format(
                    ...
                    existingProfileDesc.isEmpty() ? "无" : existingProfileDesc, currentUserInput
            );
            ...
            if (!deltaProfile.isEmpty()) {
                ...
                store.putItem(item);
                logger.info("成功增量更新用户画像, userId: {}, 更新字段: {}", userId, deltaProfile.keySet());
            }
        } catch (Exception e) {
            logger.error("提取或更新用户画像失败, userId: {}", userId, e);
        }
        return CompletableFuture.completedFuture(Map.of());
    }
    /**
    ...
    private Map<String, ProfileEntry> parseRawProfileToMap(Map<String, Object> rawProfile) {
        ...
        rawProfile.forEach((key, val) -> {
            try {
                ...
                logger.warn("画像字段 {} 解析失败，将被淘汰", key);
            }
        });
        return result;
    }
    ...
    private Map<String, String> parseJsonToStringMap(String jsonStr) {
        try {
            ...
            logger.warn("解析增量画像JSON失败，当作无更新处理。原始输出: {}", jsonStr);
